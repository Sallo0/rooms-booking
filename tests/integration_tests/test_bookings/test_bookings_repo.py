...
async def test_booking_crud(user_id, room_id):
    # Добавление брони
    new_booking = await booking_repo.add(
        user_id=user_id,
        room_id=room_id,
    ...
    new_booking = await booking_repo.find_one_or_none(id=new_booking.id)

    assert new_booking is not None

    # Удаление брони
    await booking_repo.delete(
        id=new_booking.id,
        user_id=user_id,
    )

    # Проверка удаления брони
    deleted_booking = await booking_repo.find_one_or_none(id=new_booking.id)
